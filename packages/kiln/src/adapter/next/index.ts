...
export function isNextPrunableModuleFile(mod: string): boolean {
  ...
  return (
    (mod.includes("next/dist/compiled/next-server/") && mod.endsWith(".dev.js")) ||
  ...
  return {
    ...
    getRuntimeFiles(ctx: {
      ...
      for (const file of allFiles) {
        ...
        // These files are generated by Kiln itself and must not become
        // runtime inputs on a subsequent compilation pass.
        if (posixRel === GENERATED_ASSETS_FILE || posixRel === GENERATED_SERVER_ENTRY_FILE) {
          continue;
        }
    ...
    transformStandalone(ctx: {
      ...
      const rewrittenChunks = rewriteTurbopackAliases(standaloneDir, aliases, resolutions);
