...
function compileWithBun(
  ...
  const targetArgs =
    !hasTargetFlag && targetEnv ? [`--target=${targetEnv}`] : [];
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      logError(
        "`bun` was not found on PATH. Please install it from https://bun.sh and try again.",
      );
...
export function createBunBackend(
  options: BunBackendOptions = {},
): CompilerBackend {
