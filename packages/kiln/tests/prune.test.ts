...
describe("core/prune", () => {
  ...
  describe("shouldCompressEmbeddedAsset", () => {
    ...
    it("returns false for small runtime assets below GZ_EMBED_MIN_BYTES", () => {
      expect(shouldCompressEmbeddedAsset("__runtime/server/tiny.js", GZ_EMBED_MIN_BYTES - 1, 100)).toBe(false);
    });
    ...
    it("returns false for already compressed or binary media formats", () => {
      ...
      expect(shouldCompressEmbeddedAsset("__runtime/native/addon.node", 500000, 200000)).toBe(false);
    ...
    it("returns true for qualifying runtime text files with meaningful compression ratio", () => {
      ...
      expect(shouldCompressEmbeddedAsset("__runtime/server/chunks/page.js", rawSize, gzSize)).toBe(true);
    });
  });
});
